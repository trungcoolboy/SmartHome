...
void write_led() {
  auto write_led_level = [](int level) {
    const int constrained = level < 0 ? 0 : (level > 255 ? 255 : level);
    analogWrite(NodeConfig::kLedPin, NodeConfig::kLedActiveHigh ? constrained : (255 - constrained));
  };
  ...
  if (level >= 0) {
    write_led_level(level);
    return;
  }

  if (!mqtt_client.connected()) {
    write_led_level(((millis() / 180) % 2) ? 255 : 0);
    return;
  }
  write_led_level(relay_on ? 255 : 0);
}
...
void set_relay(bool on, const char* event) {
  ...
  touch_ignore_until_ms = last_local_action_ms + kTouchIgnoreAfterRelayMs;
  last_touch_toggle_ms = last_local_action_ms;
  touch_armed = false;
...
void init_gpio() {
  ...
  touch_ignore_until_ms = 0;
  touch_release_stable_since_ms = last_touch_raw ? 0 : millis();
...
void poll_touch() {
  ...
  if (raw == touch_active) {
    if (!raw && !touch_armed) {
      if (touch_release_stable_since_ms == 0) {
        touch_release_stable_since_ms = now;
      } else if ((now - touch_release_stable_since_ms) >= kTouchRearmReleaseStableMs) {
        touch_armed = true;
      }
    } else if (raw) {
      touch_release_stable_since_ms = 0;
...
void setup() {
  ...
  delay(50);
  Serial.println();
  Serial.println("Bedroom 2 Node 01 boot");
